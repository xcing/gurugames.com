...
class TournamentController extends GeneralBaseController {
    ...
    public function actionGenaratetournament() {
        $tournamentId = Yii::app()->request->getParam('tournamentId');
        $tournamentService = new TournamentService();
        if ($tournamentService->generateTournamentBracket($tournamentId)) {
            $tournamentModel = Tournament::model()->findByPk($tournamentId);
            $tournamentModel->status = 2;
            if ($tournamentModel->save()) {
                Yii::app()->user->setFlash('success', "สร้างตารางแข่งเสร็จเรียบร้อย");
            }
        }

        $this->redirect(Yii::app()->createUrl('general/tournament/tournament/view', array('id' => $tournamentId)));
    }
