...
class SkillController extends GameBaseController {
    ...
    public function actionRanking() {
        $heroId = Yii::app()->request->getParam('heroId', 1);

        $skill_model = new Skill();
        $skills = $skill_model->findallByAttributes(
                array(
                    'heroId' => $heroId,
                ), array(
            'order' => 'ordinal asc',
    ...
    public function actionSort() {
        ...
        foreach ($records as $idxRecord => $record) {
            $skill_model->updateAll(array('ordinal' => ($idxRecord + 1)), 'skillId =:i', array(':i' => $record));
        }
        sleep(1);
    }
